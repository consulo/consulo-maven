...
final class MessageEventUtils {
    ...
    @Nonnull
    static MessageEventImpl createMessageEvent(@Nonnull Project project,
        ...
        return new MessageEventImpl(taskId, MessageEvent.Kind.ERROR, group, message, message);
    }

    @Nonnull
    private static String getExceptionText(@Nonnull Project project, @Nonnull Throwable e) {
        MavenGeneralSettings generalSettings = MavenWorkspaceSettingsComponent.getInstance(project)
        ...
        if (StringUtil.isEmpty(message)) {
            return SyncBundle.message("build.event.title.error");
        }
        return message;
    }
}
