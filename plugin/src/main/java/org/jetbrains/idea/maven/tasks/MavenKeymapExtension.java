...
public class MavenKeymapExtension implements KeymapExtension {
  public KeymapGroup createGroup(Condition<AnAction> condition, Project project) {
    KeymapGroup result = KeymapGroupFactory.getInstance().createGroup(TasksBundle.message("maven.tasks.action.group.name"), icons.MavenIcons.PhasesClosed);
    ...
    for (Map.Entry<MavenProject, Set<Pair<String, String>>> each : projectToActionsMapping.entrySet()) {
      ...
      KeymapGroup group = KeymapGroupFactory.getInstance().createGroup(mavenProject.getDisplayName(), icons.MavenIcons.PhasesClosed);
