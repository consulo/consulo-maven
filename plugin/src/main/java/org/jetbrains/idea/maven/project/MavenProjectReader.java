...
public class MavenProjectReader
{
	...
	private RawModelReadResult doReadProjectModel(VirtualFile file, boolean headerOnly)
	{
		...
		{
			parent = new MavenParent(new MavenId(MavenJDOMUtil.findChildValueByPath(xmlProject, "parent.groupId", UNKNOWN), MavenJDOMUtil.findChildValueByPath(xmlProject, "parent.artifactId",
					UNKNOWN), MavenJDOMUtil.findChildValueByPath(xmlProject, "parent.version", UNKNOWN)), MavenJDOMUtil.findChildValueByPath(xmlProject, "parent.relativePath", "../pom.xml"));
		...
		result.setMavenId(new MavenId(MavenJDOMUtil.findChildValueByPath(xmlProject, "groupId", parent.getMavenId().getGroupId()), MavenJDOMUtil.findChildValueByPath(xmlProject, "artifactId",
				UNKNOWN), MavenJDOMUtil.findChildValueByPath(xmlProject, "version", parent.getMavenId().getVersion())));
	...
	private static List<MavenResource> collectResources(List<Element> xmlResources)
	{
		...
		{
			result.add(new MavenResource(MavenJDOMUtil.findChildValueByPath(each, "directory"), "true".equals(MavenJDOMUtil.findChildValueByPath(each, "filtering")),
					MavenJDOMUtil.findChildValueByPath(each, "targetPath"), MavenJDOMUtil.findChildrenValuesByPath(each, "includes", "include"), MavenJDOMUtil.findChildrenValuesByPath(each,
					"excludes", "exclude")));
		}
		return result;
	}
	...
	private List<MavenResource> repairResources(List<MavenResource> resources, String defaultDir)
	{
		...
		{
			if(isEmptyOrSpaces(each.getDirectory()))
			{
				continue;
			}
			result.add(each);
	...
	private List<MavenProfile> collectProfiles(VirtualFile projectFile, Element xmlProject, Collection<MavenProjectProblem> problems, Set<String> alwaysOnProfiles)
	{
		...
		{
			collectProfilesFromSettingsXmlOrProfilesXml(profilesFile, "profilesXml", true, MavenConstants.PROFILE_FROM_PROFILES_XML, result, alwaysOnProfiles, problems);
		}
	...
	private void collectProfiles(List<Element> xmlProfiles, List<MavenProfile> result, String source)
	{
		for(Element each : xmlProfiles)
		{
			...
			if(xmlActivation != null)
			{
				...
				if(xmlOS != null)
				{
					activation.setOs(new MavenProfileActivationOS(MavenJDOMUtil.findChildValueByPath(xmlOS, "name"), MavenJDOMUtil.findChildValueByPath(xmlOS, "family"),
							MavenJDOMUtil.findChildValueByPath(xmlOS, "arch"), MavenJDOMUtil.findChildValueByPath(xmlOS, "version")));
				}
	...
	private boolean addProfileIfDoesNotExist(MavenProfile profile, List<MavenProfile> result)
	{
		...
		{
			if(Comparing.equal(each.getId(), profile.getId()))
			{
				return false;
			}
		}
		...
		try
		{
			...
			Pair<VirtualFile, RawModelReadResult> parentModelWithProblems = new MavenParentProjectFileProcessor<Pair<VirtualFile, RawModelReadResult>>()
			{
				...
				protected Pair<VirtualFile, RawModelReadResult> processRelativeParent(VirtualFile parentFile)
				{
					...
					if(!parentId.equals(parentModel.getMavenId()))
					{
						return null;
					}
			...
			if(parentModelWithProblems == null)
			{
				return model; // no parent or parent not found;
			}

			MavenModel parentModel = parentModelWithProblems.second.model;
			if(!parentModelWithProblems.second.problems.isEmpty())
			{
				problems.add(MavenProjectProblem.createProblem(parentModelWithProblems.first.getPath(), ProjectBundle.message("maven.project.problem.parentHasProblems", parentModel.getMavenId()),
			...
			MavenProjectReaderProjectLocator locator) throws MavenProcessCanceledException
	{
		try
		{
			...
			return new MavenProjectReaderResult(projectData.mavenModel, projectData.mavenModelMap, new MavenExplicitProfiles(projectData.activatedProfiles, explicitProfiles.getDisabledProfiles()),
					projectData.nativeMavenProject, result.problems, result.unresolvedArtifacts);
		}
			...
			MavenConsole console) throws MavenProcessCanceledException
	{
		try
		{
			...
			return new MavenProjectReaderResult(projectData.mavenModel, projectData.mavenModelMap, new MavenExplicitProfiles(projectData.activatedProfiles, profiles.getDisabledProfiles()),
					projectData.nativeMavenProject, result.problems, result.unresolvedArtifacts);
		}
		catch(Throwable e)
		{
