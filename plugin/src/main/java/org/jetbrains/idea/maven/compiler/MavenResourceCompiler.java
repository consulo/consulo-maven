...
public class MavenResourceCompiler implements ClassPostProcessingCompiler
{
	...
	private void loadCache(final Project project)
	{
		...
		try
		{
			...
			try
			{
				if(in.readInt() != CompileDriver.DEPENDENCY_FORMAT_VERSION)
				{
					return;
				}
			...
			List<MyProcessingItem> result)
	{
		...
		for(MavenResource each : resources)
		{
			...
			String resourceOutputDir = StringUtil.isEmptyOrSpaces(targetPath) ? outputDir : (FileUtil.isAbsolute(targetPath) ? targetPath : outputDir + "/" + targetPath);

			collectProcessingItems(module, dir, dir, resourceOutputDir, includes, excludes, each.isFiltered(), properties, propertiesHashCode, nonFilteredExtensions, escapeString, result, context
	...
	public ProcessingItem[] process(final CompileContext context, ProcessingItem[] items)
	{
		...
		for(int i = 0; i < items.length; i++)
		{
			...
			try
			{
				...
				{
					context.addMessage(CompilerMessageCategory.WARNING, "Maven: File is too big to be filtered. Most likely it is a binary file and should be excluded from filtering.",
							sourceVirtualFile.getPath(), -1, -1);
					shouldFilter = false;
				}
			...
			{
				MavenLog.LOG.info(e);
				context.addMessage(CompilerMessageCategory.ERROR, "Maven: Cannot process resource file: " + e.getMessage(), sourceVirtualFile.getPath(), -1, -1);
			}
		}
