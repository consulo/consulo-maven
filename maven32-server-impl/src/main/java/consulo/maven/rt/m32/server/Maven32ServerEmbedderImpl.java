...
import consulo.maven.rt.m3.common.server.*;
import consulo.maven.rt.m3.common.server.embedder.MavenExecutionResult;
import consulo.maven.rt.m3.common.server.embedder.*;
import consulo.maven.rt.m32.server.embedder.CustomMaven32ArtifactResolver;
...
public class Maven32ServerEmbedderImpl extends Maven3ServerEmbedder {
    ...
    public static ProfileApplicationResult applyProfiles(MavenModel model, File basedir, MavenExplicitProfiles explicitProfiles, Collection<String> alwaysOnProfiles) throws RemoteException {
        ...
        return new ProfileApplicationResult(MavenModelConverter.convertModel(nativeModel, null), new MavenExplicitProfiles(collectProfilesIds(activatedProfiles),
    ...
    public MavenServerExecutionResult resolveProject(@Nonnull File file,
        ...
        MavenExecutionResult result = doResolveProject(file, new ArrayList<String>(activeProfiles), new ArrayList<String>(inactiveProfiles), Arrays.<ResolutionListener>asList(listener));
        return createExecutionResult(file, result, listener.getRootNode());
    }

    @Nullable
    @Override
    public String evaluateEffectivePom(@Nonnull File file, @Nonnull List<String> activeProfiles, @Nonnull List<String> inactiveProfiles) throws RemoteException, MavenServerProcessCanceledException {
    ...
    public MavenExecutionRequest createRequest(File file, List<String> activeProfiles, List<String> inactiveProfiles, List<String> goals) throws RemoteException {
        ...
        try {
            ...
            if (result.getExecutionListener() == null) {
                result.setExecutionListener(new AbstractExecutionListener());
            }
    ...
    private MavenServerExecutionResult createExecutionResult(File file, MavenExecutionResult result, DependencyNode rootNode) throws RemoteException {
        ...
        try {
            if (USE_MVN2_COMPATIBLE_DEPENDENCY_RESOLVING) {
                //noinspection unchecked
                final List<DependencyNode> dependencyNodes = rootNode == null ? Collections.<DependencyNode>emptyList() : rootNode.getChildren();
                model = MavenModelConverter.convertModel(mavenProject.getModel(), mavenProject.getCompileSourceRoots(), mavenProject.getTestCompileSourceRoots(), mavenProject.getArtifacts(),
                    dependencyNodes, mavenProject.getExtensionArtifacts(), getLocalRepositoryFile());
            }
            else {
                final DependencyResolutionResult dependencyResolutionResult = result.getDependencyResolutionResult();
                final org.eclipse.aether.graph.DependencyNode dependencyGraph = dependencyResolutionResult != null ? dependencyResolutionResult.getDependencyGraph() : null;
    ...
    @Nonnull
    @Override
    public MavenArtifact resolve(@Nonnull MavenArtifactInfo info, @Nonnull List<MavenRemoteRepository> remoteRepositories) throws RemoteException, MavenServerProcessCanceledException {
    ...
    private MavenExecutionResult doExecute(@Nonnull final File file,
        ...
        final MavenExecutionRequest request = createRequest(file, activeProfiles, inactiveProfiles, goals);
        ...
        final org.apache.maven.execution.MavenExecutionResult[] resultRef = new org.apache.maven.execution.MavenExecutionResult[1];
        executeWithMavenSession(request, new Runnable() {
            @Override
            public void run() {
                try {
                    resultRef[0] = safeExecute(request, getComponent(Maven.class));
                }
                catch (RemoteException e) {
                    throw new RuntimeException(e);
                }
        ...
        org.apache.maven.execution.MavenExecutionResult executionResult = resultRef[0];
