...
import consulo.maven.rt.server.common.util.MavenReflectionUtil;
...
public class Maven2ModelConverter
{
	...
	public static MavenModel convertModel(Model model, File localRepository) throws RemoteException
	{
		...
		return convertModel(model, asSourcesList(build.getSourceDirectory()), asSourcesList(build.getTestSourceDirectory()), Collections.<Artifact>emptyList(), Collections.<DependencyNode>emptyList
				(), Collections.<Artifact>emptyList(), localRepository);
	}
	...
	public static MavenArtifact convertArtifact(Artifact artifact, File localRepository)
	{
		return new MavenArtifact(artifact.getGroupId(), artifact.getArtifactId(), artifact.getVersion(), artifact.getBaseVersion(), artifact.getType(), artifact.getClassifier(),
				...
				artifact.getFile(), localRepository,

				artifact.isResolved(), artifact instanceof CustomArtifact && ((CustomArtifact) artifact).isStub());
	}
	...
	private static List<MavenPlugin> convertPlugins(Model mavenModel) throws RemoteException
	{
		...
		if(build != null)
		{
			doConvertPlugins(build.getPluginManagement(), true, result, pluginKeys);
		}
		return result;
	}
	...
	private static MavenPlugin convertPlugin(boolean isDefault, Plugin plugin) throws RemoteException
	{
		...
		for(Object each : plugin.getExecutions())
		{
			executions.add(convertExecution((PluginExecution) each));
		}

		List<MavenId> deps = new ArrayList<MavenId>(plugin.getDependencies().size());
		for(Object each : plugin.getDependencies())
		{
			Dependency dependency = (Dependency) each;
			deps.add(new MavenId(dependency.getGroupId(), dependency.getArtifactId(), dependency.getVersion()));
		}

		return new MavenPlugin(plugin.getGroupId(), plugin.getArtifactId(), plugin.getVersion(), isDefault, plugin.isExtensions(), convertConfiguration(plugin.getConfiguration()), executions, deps);
	}
	...
	public static Repository toNativeRepository(MavenRemoteRepository r)
	{
		...
		{
			result.setReleases(toNativePolicy(r.getReleasesPolicy()));
		}
		if(r.getSnapshotsPolicy() != null)
		{
			result.setSnapshots(toNativePolicy(r.getSnapshotsPolicy()));
		}
