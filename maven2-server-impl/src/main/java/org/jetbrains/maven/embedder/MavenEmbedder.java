...
public class MavenEmbedder
{
	...
	public MavenExecutionResult resolveProject(@Nonnull final File file, @Nonnull final List<String> activeProfiles, @Nonnull final List<String> inactiveProfiles, List<ResolutionListener> listeners)
	{
		...
		try
		{
			...
			ArtifactResolutionResult result = resolver.resolveTransitively(project.getDependencyArtifacts(), projectArtifact, managedVersions, myLocalRepository,
	...
	public Set<Artifact> resolveTransitively(@Nonnull Set<Artifact> toResolve, @Nonnull List<ArtifactRepository> repos) throws ArtifactResolutionException, ArtifactNotFoundException
	{
		...
		return getComponent(ArtifactResolver.class).resolveTransitively(toResolve, project, Collections.EMPTY_MAP, myLocalRepository, repos,
	...
	private MavenExecutionRequest createRequest(File file, List<String> activeProfiles, List<String> inactiveProfiles, List<String> goals)
	{
		...
		MavenExecutionRequest result = new DefaultMavenExecutionRequest(myLocalRepository, mySettings, dispatcher, goals, file.getParent(), createProfileManager(activeProfiles, inactiveProfiles,
				executionProperties), executionProperties, new Properties(), true)
		{
