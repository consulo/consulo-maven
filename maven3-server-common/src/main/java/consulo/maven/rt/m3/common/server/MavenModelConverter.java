...
import consulo.maven.rt.m3.common.server.embedder.CustomMaven3Artifact;
...
public class MavenModelConverter
{
	...
	public static MavenArtifact convertArtifact(Artifact artifact, File localRepository)
	{
		return new MavenArtifact(artifact.getGroupId(), artifact.getArtifactId(), artifact.getVersion(), artifact.getBaseVersion(), artifact.getType(), artifact.getClassifier(),
				...
				artifact.isResolved(), artifact instanceof CustomMaven3Artifact && ((CustomMaven3Artifact) artifact).isStub());
	}
	...
	private static MavenPlugin convertPlugin(boolean isDefault, Plugin plugin) throws RemoteException
	{
		...
		return new MavenPlugin(plugin.getGroupId(), plugin.getArtifactId(), plugin.getVersion(), isDefault, "true".equals(plugin.getExtensions()), convertConfiguration(plugin.getConfiguration()),
				executions, deps);
	}
